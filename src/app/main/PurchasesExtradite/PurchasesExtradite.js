...
import {
    AddOrUpdateEnum, useAddOrUpdateInvoiceHMutation, useGetInvoiceHByTypesQuery, useGetAcAccountDocumentHLazyQuery,
    useGetCurrenciesQuery, useDeleteInvoicehMutation, useGetInvoicehWithFilterForReturnLazyQuery, useGetInvoiceTypesQuery
...
function PurchasesExtradite() {
    ...
    const gTotalCalculation = () => {
        let gTotalAmount = 0;
        gTotalAmount = (subTotal + sctamount + vatamount);
        return parseFloat(gTotalAmount);
    }

    for (let i = 0; i < rowData?.acAccountdocumenths?.length; i++) {
        totalPaidAmount += rowData?.acAccountdocumenths[i].amount;
    }
    ...
    const GetInvoicehForReturn = () => {
        getInvoicehWithFilterForReturn({
            variables: {
                ...
                prmInvoiceType: filterParams.invoicetype === "" ? "PU" : filterParams.invoicetype,
                prmInvoiceType2: filterParams.invoicetype === "" ? "PR" : "",
        ...
        <AutoForm
            ...
            onSubmit={model => {
                ...
                for (let i = 0; i < model?.invoiceds?.length; i++) {
                    console.log(model?.invoiceds[i], "ii", model?.invoiceds);
                    let indexes = []
            ...
            <MSCToolbar
                ...
                headerName={t('PurchasesExtradite')}
                ...
                ReadModeHeaderComponnent={<div>
                    ...
                    >
                        <MenuItem onClick={() => MenuButtonClick(selectedData, AddOrUpdateEnum.Add)}
                            disabled={selectedData?.invoicetype === "PU" ? isReturnedAll : true}
                        >{t('PurchaseExtradite')}</MenuItem>
                    </Menu>
                </div>}
                ReadComponnent={
                    <div style={{ width: '100%' }}>
                        <MSCReactTable
                            ...
                            columns={[
                                ...
                                {
                                    accessor: (row) => { if (row.invoicetype === "PU") { return t('PurchaseInvoice') } else { return t('PurchaseExtradite') } },
                ...
                CreateOrUpdateComponnent={
                    <div>
                        <>
                            ...
                            <HiddenField name={'invoicedoctype'} value={"PR"} />
                            <HiddenField name={'invoicetype'} value={"PR"} />
                        ...
                        <Grid container spacing={2}>
                            ...
                            {
                                getViewModeEx === 3 && selectedData?.invoicetype === "PU" ?
                                    <Grid item xs={12} sm={12} lg={12} md={12}>
                                        <Card >
                                            <CardContent>
                                                ...
                                                <Typography style={{ float: 'right', padding: '2%' }} variant='subtitle1'
                                                    fontWeight={'bold'}>{t('TotalPaidAmount')} : {MSCCurrencyFormatter(totalPaidAmount,rowData?.currency)}</Typography>
                                            </CardContent>
                                        </Card>
                                    </Grid>
                                    : null}
...
export default PurchasesExtradite; 